 ...
 * selection.
 */
export const BoxList: React.FC<BoxListProps> = ({ engine, snap, onClose }) => {
  ...
  const visible = filter === null ? boxes : boxes.filter((b) => b.label === filter);
  const selectedIds = useMemo(() => new Set(snap.selection.map((b) => b.id)), [snap.selection]);
  ...
  useEffect(() => {
    ...
    listRef.current?.querySelector(`[data-box-id="${CSS.escape(firstSelected)}"]`)?.scrollIntoView({ block: "nearest" });
  }, [firstSelected]);
  ...
  return (
    <aside className="w-72 shrink-0 border-l border-gray-200 flex flex-col min-h-0 text-xs bg-white">
      <div className="flex items-center justify-between px-3 h-9 border-b border-gray-200">
        <span className="font-semibold text-sm">Boxes <span className="text-gray-400 font-normal">{boxes.length}</span></span>
        <button className="p-1 rounded hover:bg-gray-100" onMouseDown={keepFocus} onClick={onClose} title="Close panel">
      ...
      {conflicts.length > 0 && (
        <div className="px-3 py-2 border-b border-gray-200 bg-red-50 flex items-center gap-2">
          ...
          <button className="text-red-700 underline text-left" onMouseDown={keepFocus} onClick={() => engine.focusConflict(1)} title="Go to the next conflict (F8)">
      ...
      {counts.length > 0 && (
        <div className="px-3 py-2 border-b border-gray-200 flex flex-wrap gap-1">
          ...
          {counts.map(([label, count]) => (
            <span key={label} className={`flex items-center gap-0.5 rounded ${filter === label ? "ring-2 ring-gray-500" : ""}`}>
              <LabelChip label={label} onClick={() => setFilter(filter === label ? null : label)} title={`Show only ${label}`} />
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto">
        {visible.length === 0 ? (
          <p className="p-3 text-gray-400">No boxes yet — drag on the spectrogram to add one.</p>
        ) : (
          visible.map((box) => (
            <BoxRow
              key={box.id}
...
const BoxRow = React.memo<{ box: EditorBox; selected: boolean; conflicted: boolean; onClick: (toggle: boolean) => void }>(
  ({ box, selected, conflicted, onClick }) => (
    <div
      ...
      className={`flex items-center gap-2 px-3 py-1 cursor-pointer tabular-nums border-l-2 ${
        selected ? "bg-amber-50 border-amber-500" : "border-transparent hover:bg-gray-50"
      }`}
    >
      <LabelChip label={box.label} />
      <span className="w-16">{formatTime(box.start, 0.001)}</span>
      <span className="w-14 text-gray-500">{formatDuration(box.end - box.start)}</span>
      <span className="flex-1 text-gray-400 truncate">
        {isTimeSegment(box) ? "all Hz" : `${(box.fLow! / 1000).toFixed(1)}–${(box.fHigh! / 1000).toFixed(1)}k`}
      </span>
      {conflicted && <ExclamationTriangleIcon className="h-3.5 w-3.5 text-red-600" title="Conflict" />}
    </div>
  ),
  // Re-render a row only when its own data changes.
  (a, b) => a.box === b.box && a.selected === b.selected && a.conflicted === b.conflicted,
);
