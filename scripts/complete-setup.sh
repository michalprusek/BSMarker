...
if [ -d "certbot/conf/live/bsmarker.utia.cas.cz" ]; then
    echo -e "${GREEN}✓ SSL certificates already exist${NC}"

    # Check expiry
    if [ -f "certbot/conf/live/bsmarker.utia.cas.cz/fullchain.pem" ]; then
        EXPIRY=$(openssl x509 -in certbot/conf/live/bsmarker.utia.cas.cz/fullchain.pem -noout -enddate | cut -d= -f2)
...
if [[ "$response" =~ ^([yY][eE][sS]|[yY]| )$ ]] || [ -z "$response" ]; then
    ...
    echo -e "${MAGENTA}   https://bsmarker.utia.cas.cz${NC}"
    echo ""
    ...
    echo -e "   Health check:     ${YELLOW}curl https://bsmarker.utia.cas.cz/health${NC}"
    echo -e "   API health:       ${YELLOW}curl https://bsmarker.utia.cas.cz/api/v1/health${NC}"
    echo ""
else
