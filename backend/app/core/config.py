...
class Settings(BaseSettings):
    ...
    REDIRECT_HOST: str = Field(
        default="bsmarker.utia.cas.cz", description="Host to use in redirect location headers"
    )
    ...
    def validate_database_url(cls, v: str) -> str:
        ...
        if not re.match(r"postgresql://[^:]+:[^@]+@[^:/]+:\d+/[^/]+", v):
            raise ValueError(
                "DATABASE_URL format invalid. Expected: postgresql://user:pass@host:port/db"
            )
