...
def merge_configurations(existing: Dict[str, str], new_secrets: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "REDIS_PASSWORD" in new_secrets:
        ...
        merged["REDIS_URL"] = (
            f"redis://:{new_secrets['REDIS_PASSWORD']}@{redis_host}:{redis_port}/{redis_db}"
        )
...
def write_rotation_summary(backup_path: Path, rotated_keys: set) -> None:
    ...
    """
    summary_path = (
        backup_path.parent / f"rotation_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
...
def main():
    ...
    parser.add_argument(
        "--env-file", "-e", type=str, default=".env", help="Path to .env file (default: .env)"
    )
    parser.add_argument(
        "--preserve-admin", action="store_true", help="Preserve admin password (useful for testing)"
    )
    parser.add_argument(
