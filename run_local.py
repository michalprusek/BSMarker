...
def check_and_handle_ports(force=False):
    ...
    ports_to_check = {FRONTEND_PORT: "Frontend (React)", BACKEND_PORT: "Backend (FastAPI)"}
...
def check_python_requirements():
    ...
    try:
        ...
        try:
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "-r", "system_requirements.txt"],
                check=True,
                capture_output=True,
            )
...
def check_requirements():
    ...
    requirements = {"python3": "Python 3.11+", "psql": "PostgreSQL", "redis-cli": "Redis"}
...
def setup_database():
    ...
    for cmd in commands:
        try:
            subprocess.run(
                ["psql", "-U", os.getenv("USER"), "-d", "postgres", "-c", cmd],
                capture_output=True,
                check=False,
            )
        except:
...
"""
        ...
        print(
            "🔐 SECURITY WARNING: Run 'cd backend && python app/core/generate_secrets.py' to generate secure secrets"
        )
...
def start_minio():
    ...
    # Start MinIO server
    minio_cmd = ["minio", "server", str(minio_data), "--console-address", ":9001"]
...
Examples:
  ...
  python3 run_local.py --stop       # Stop running instance
    ...
    parser.add_argument(
        "--force", action="store_true", help="Automatically kill processes on conflicting ports"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show current BSMarker status and exit"
    )
    parser.add_argument(
        "--stop", action="store_true", help="Stop running BSMarker instance and exit"
    )
    parser.add_argument("--version", action="version", version=f"BSMarker Local Runner v{VERSION}")
